...
void Puntaje::calculoPuntajes()
{
    ...
    for(unsigned int i=0; i<(unsigned)infoArchivoUsuarios.size(); i++){
        qDebug() << QString::fromStdString(infoArchivoUsuarios.at(i).string1)
                 << QString::fromStdString(infoArchivoUsuarios.at(i).string2)
                 << infoArchivoUsuarios.at(i).int1
                 << infoArchivoUsuarios.at(i).int2;
...
{
    ordenarPuntajes();
    qDebug() << "DATOS ORDENADOS: ";
    for(unsigned int i=0; i<(unsigned)MaxPuntajes.size(); i++){
        qDebug() << QString::fromStdString(MaxPuntajes.at(i).nombre)
                 << MaxPuntajes.at(i).puntaje;
    }
    dibujarPuntajes();
}
